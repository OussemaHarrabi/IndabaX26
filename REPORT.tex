...
\date{IndabaX Tunisia 2026\\Engineering snapshot: 24 September 2026}
...
Each canonical observation retains kind, content, source, trust level, and sensitivity. Trust is ordered from system policy and authenticated users through trusted internal sources to untrusted internal, untrusted external, and adversary-controlled material; the exact enum names are in the contract source. Sensitivity ranges from public through restricted. Each provenance source is expanded independently so a trusted co-source cannot mask an untrusted source. A reference that is explicitly missing, duplicated, or ambiguous is represented adversary-controlled and restricted. Evidence with no provenance identifiers is marked unattributed/untrusted; this differs from a broken integrity reference and is not silently upgraded.
...
Benchmark source & \texttt{Skan22/Sentinel\_Starter\_Kit}\newline\texttt{dd2e5fe0979d0781a4bfe6d0849cd80cf69ef4a2} \\
...
The complete \texttt{allow\_all} pass completed 40/40 scenarios (31 attacks, 9 benign). The reachability validator passed only 22 attacks: ten enterprise, five finance, and seven SOC cases. The other nine are not defense-effectiveness evidence. The committed allow-all scorecard and reachability manifest identify every case individually; do not interpret an unreached attack's defended outcome as successful blocking.

The full-suite allow-all metrics are ASR 0.7097 (22/31), BTU 0.5556 (5/9), CVR 0.5500, DFI 0.4500, FBR 0.0000, UER 0.0000, TUI 0.7097, Brier 0.1831, ECE 0.1831, and p95 defense-decision latency 0.056 ms. Its scorecard hash and deterministic evaluator digest are in the evidence manifest. These are model/task baseline measurements, not defense effectiveness; full-suite denominators include the nine unreachable attacks. Per-domain allow-all ASR is enterprise 0.8333 (10/12), finance 0.5556 (5/9), SOC 0.7000 (7/10). Benign task success is 1/3 for finance, 1/3 for SOC, and 3/3 for enterprise. This poor baseline utility is an important limitation and motivates careful matched analysis, not metric suppression.
...
The final v5 archive (named in the evidence manifest) contains the scorecard, metadata, logs, and 40 raw JSONL traces (90 entries). Its integrity values are:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,frame=none]
archive sha256   37dbcf836108ad667c720666d27e63c8d8b1001d1ea93ab53a2e84ce7f702d76
scorecard sha256 b9b0937814f8a545b8cb1deebacb1623830a04dedbfdb6b8f297be31118027c4
evaluator digest 57ad9925d63d735eb27bddc8e6d23338c076308e20a657e135522146482e57a5
\end{lstlisting}
The prior three-run and v3 archives are retained for ablation and baseline comparison; see \texttt{evaluation/real-qwen/README.md} for their hashes. The matching synthetic artifacts are also in an owner-authorized view-link Drive folder: \url{https://drive.google.com/drive/u/0/folders/19gLoN8kqGqdClsYWYUMSSlexLLZ0RlHK}. The repository copy is the evidence of record. Synthetic canaries are not real credentials.
...
\begin{longtable}{p{0.21\linewidth}p{0.48\linewidth}p{0.22\linewidth}}
\toprule
Gate & Evidence required & Status \\
\midrule
Build and package & Clean checkout builds; assets included; pinned dependencies install & Partial \\
Automated QA & Tests, Ruff, mypy, JS syntax pass on frozen commit & Passed on v5; rerun before release \\
Dashboard evidence & Real JSONL + scorecard import, filters, comparison warnings, related navigation, export, clear/undo & Desktop/narrow checked; assistive tech pending \\
Reachability & Raw allow-all records for all attacks, with every showcased scenario true & 22/31 reached; two demo cases checked \\
Matched baseline comparison & Allow-all, provenance, AegisGraph v1/v3/v5 under same reference settings & Complete; one seed \\
Security boundary & Invalid/missing provenance, oversized payload, malformed action, rewrite trap, external recipient, missing confirmation all block/escalate as designed & Automated tests; release rerun due \\
Failure behavior & Safe block on internal evaluation exception; no tool/model/network side effects & Local tests pass; external enforcement needed \\
Container & Build, non-root, read-only filesystem, dropped capabilities, no-new-privileges, resource limits & Docker check pending \\
Official validator & Manifest + live health/decision endpoint accepted by pinned kit validator & Pending \\
Human eligibility & Organizer confirms required 3--5 participants or accepts the team's registration & Pending \\
Demo & 5--10 minutes; one reached-and-stopped attack, one benign success, inspectable actual trace/outcome & Pending \\
Submission & Final repo freeze, LaTeX report, artifact hashes, video, portal receipt & PDF ready; video/portal pending \\
\bottomrule
\end{longtable}
...
The repository README defines the implementation contract and points to the dashboard brief, benchmark lock, Colab run protocol, evidence manifests, demo runbook, and release checklist. The starter kit's pinned research template, scoring guide, participant guide, threat model, and security model remain authoritative for organizer-specific requirements.
